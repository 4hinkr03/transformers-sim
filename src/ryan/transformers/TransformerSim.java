...
public class TransformerSim extends Simulator {
	...
	protected void update() {
		if (step % TransformerConfig.MAX_PATH == 0) {
			...
			List<AutoBot> selection = selection();

			// clear down the environment
			clear();

			// add the new autobots selection
			addAll(selection);
	...
	private List<AutoBot> generateGenePool() {
		...
		int totalSize = 0;
		for (AutoBot bot : bots) {
			// calculate the number of autobots to add to the gene pool
			// the greater the fitness, the more autobots added to the gene pool
			int botSize = (int) (bot.getFitness() * 100 * TransformerConfig.MAX_TRANSFORMERS);
			// add the number of calculated autobots to the gene pool
	...
	/**
	 * Merge the paths of the mum and dad to produce a path based on the parents
	 * genes
	 *
